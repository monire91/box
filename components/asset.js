...
const Asset = ({asset}) => {
    ...
    for (let i = 0; i < state.transactionTypes.length; i++) {
        console.log(state.transactionTypes[i])
    }
    ...
    return (
            <div onClick={handleClick} className='w-full sm-w-1/2 md:w-1/3 xl:w-1/5 ml-8 mr-10 mt-10 border-2 rounded-3xl overflow-hidden border-gray-300'>
                <div className='overflow-hidden h-tall relative w-full group'>
                    ...
                    <div>
                        ...
                        <span
                            className='transition-all duration-300 ease-out rounded text-white p-2 text-xs bg-secondary absolute ml-6 mt-32 group-hover:opacity-0 transition transform group-hover:-translate-y-16'>بازار ثانویه</span>
                    </div>
